...
final class CachedIntrospectionResults {
    ...
    protected static CachedIntrospectionResults forClass(Class clazz) throws BeansException {
        ...
        if (results == null) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Using cached introspection results for class " + clazz.getName());
            }
        }
    ...
    private CachedIntrospectionResults(Class clazz) throws FatalBeanException {
        try {
            log.debug("Getting BeanInfo for class [" + clazz.getName() + "]");
            this.beanInfo = Introspector.getBeanInfo(clazz);

            log.debug("Caching PropertyDescriptors for class [" + clazz.getName() + "]");
            this.propertyDescriptorMap = new HashMap();
            // This call is slow so we do it once
            PropertyDescriptor[] pds = this.beanInfo.getPropertyDescriptors();
            for (int i = 0; i < pds.length; i++) {
                log.debug("Found property '" + pds[i].getName() + "' of type [" + pds[i].getPropertyType() +
